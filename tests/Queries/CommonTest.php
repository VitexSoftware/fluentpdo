...
class CommonTest extends TestCase
{
    ...
    public function testShortJoin(): void
    {
    ...
    public function testJoinShortBackRef(): void
    {
        ...
        self::assertEquals(
            'SELECT user.* FROM user INNER JOIN article AS with_articles ON with_articles.user_id = user.id',
    ...
    public function testJoinShortMulti(): void
    {
        $query = $this->fluent->from('comment')
            ->leftJoin('article.user');

        self::assertEquals(
            'SELECT comment.* FROM comment LEFT JOIN article ON article.id = comment.article_id  LEFT JOIN user ON user.id = article.user_id',
            $query->getQuery(false),
        );
    }

    public function testJoinMultiBackRef(): void
    {
        $query = $this->fluent->from('article')
            ->innerJoin('comment:user AS comment_user');

        self::assertEquals(
            'SELECT article.* FROM article INNER JOIN comment ON comment.article_id = article.id  INNER JOIN user AS comment_user ON comment_user.id = comment.user_id',
            $query->getQuery(false),
        );
        self::assertEquals(
    ...
    public function testJoinShortTwoSameTable(): void
    {
        $query = $this->fluent->from('article')
            ->leftJoin('user')
            ->leftJoin('user');
    ...
    public function testJoinShortTwoTables(): void
    {
        $query = $this->fluent->from('comment')
            ->where('comment.id', 2)
    ...
    public function testJoinInWhere(): void
    {
        $query = $this->fluent->from('article')->where('comment:content <> "" AND user.country.id = ?', 1);

        self::assertEquals(
            'SELECT article.* FROM article LEFT JOIN comment ON comment.article_id = article.id  LEFT JOIN user ON user.id = article.user_id  LEFT JOIN country ON country.id = user.country_id WHERE comment.content <> "" AND country.id = ?',
            $query->getQuery(false),
        );
    }

    public function testJoinInSelect(): void
    {
    ...
    public function testJoinInOrderBy(): void
    {
        $query = $this->fluent->from('article')->orderBy('user.name, article.title');

        self::assertEquals(
            'SELECT article.* FROM article LEFT JOIN user ON user.id = article.user_id ORDER BY user.name, article.title',
            $query->getQuery(false),
        );
    }

    public function testJoinInGroupBy(): void
    {
        $query = $this->fluent->from('article')->groupBy('user.type')
            ->select(null)->select('user.type, count(article.id) AS article_count');

        self::assertEquals(
            'SELECT user.type, count(article.id) AS article_count FROM article LEFT JOIN user ON user.id = article.user_id GROUP BY user.type',
            $query->getQuery(false),
        );
        self::assertEquals(
            ['0' => ['type' => 'admin', 'article_count' => '4'], '1' => ['type' => 'author', 'article_count' => '2']],
            $query->fetchAll(),
        );
    }

    public function testEscapeJoin(): void
    {
        $query = $this->fluent->from('article')
        ...
        self::assertEquals(
            'SELECT article.* FROM article LEFT JOIN comment ON comment.id = article.comment_id WHERE comment.id = :id AND user.name = :name',
            $query->getQuery(false),
        );
    }

    public function testDontCreateDuplicateJoins(): void
    {
        $query = $this->fluent->from('article')
        ...
        self::assertEquals(
            'SELECT article.*, author.name FROM article INNER JOIN user AS author ON article.user_id = author.id',
            $query->getQuery(false),
        );
        self::assertEquals('SELECT article.*, user.name FROM article INNER JOIN user ON article.user_id = user.id', $query2->getQuery(false));
        self::assertEquals(
            'SELECT article.*, country.name FROM article INNER JOIN user AS author ON article.user_id = author.id  LEFT JOIN country ON country.id = author.country_id',
            $query3->getQuery(false),
        );
        self::assertEquals(
            'SELECT article.*, country.name FROM article INNER JOIN user ON article.user_id = user.id  LEFT JOIN country ON country.id = user.country_id',
            $query4->getQuery(false),
        );
    }

    public function testClauseWithRefBeforeJoin(): void
    {
        ...
        self::assertEquals(
            'SELECT article.*, author.name FROM article INNER JOIN user AS author ON author.id = article.user_id',
            $query2->getQuery(false),
        );
    ...
    public function testFromOtherDB(): void
    {
    ...
    public function testJoinTableWithUsing(): void
    {
        $query = $this->fluent->from('article')
    ...
    public function testDisableSmartJoin(): void
    {
        $query = $this->fluent->from('comment')
            ->select('user.name')
        ...
        self::assertEquals(
            '-- Plain: SELECT comment.*, user.name FROM comment LEFT JOIN user ON user.id = comment.user_id  LEFT JOIN article ON article.id = comment.article_id ORDER BY article.published_at',
            $printQuery,
        );
        self::assertEquals('-- Disable: SELECT comment.*, user.name FROM comment ORDER BY article.published_at', $printQuery2);
        self::assertEquals(
            '-- Disable and enable: SELECT comment.*, user.name FROM comment LEFT JOIN user ON user.id = comment.user_id  LEFT JOIN article ON article.id = comment.article_id ORDER BY article.published_at',
            $printQuery3,
        );
    }

    public function testPDOFetchObj(): void
    {
    ...
    public function testFromIdAsObject(): void
    {
    ...
    public function testFromIdAsObjectUser(): void
    {
